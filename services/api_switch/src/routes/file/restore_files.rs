...
pub async fn handle(
    ...
    // Shadow (anonymous) sessions may restore their own files, so any caller is allowed.
    ...
    match file_query {
        Ok(Some(record)) => {
            ...
            let restore_result = sqlx::query!(
                "UPDATE files SET deleted = false, deleted_on = NULL WHERE id = $1",
                payload.file_id
            )
            ...
            match restore_result {
                Ok(_) => respond(200, "File restored successfully", vec![], json!({})),
                Err(_e) => respond(500, "Failed to restore file", vec![], json!({})),
            }
        },
